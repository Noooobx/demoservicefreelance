import React, { memo } from 'react';
import { Star, ArrowRight } from 'lucide-react';
import { useIntersection } from '../hooks/useIntersection';
import { cn } from '../utils/cn';

const GoogleReviews = memo(() => {
  const [ref, isIntersecting] = useIntersection({ once: true, threshold: 0.1 });

  return (
    <section id="reviews" ref={ref} className="py-16 bg-white border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-6 flex flex-col items-center text-center">
        <div
          className={cn(
            "bg-accent/5 p-12 rounded-[40px] border border-accent/10 relative overflow-hidden group transition-all duration-300 reveal",
            isIntersecting && "reveal-active"
          )}
        >
          {/* Subtle background glow */}
          <div className="absolute inset-0 bg-accent/10 blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
          
          <div className="relative z-10">
            <div className="flex items-center justify-center gap-2 mb-4">
              <svg className="w-8 h-8" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-1 .67-2.28 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" />
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
              </svg>
              <span className="text-2xl font-bold text-primary">Google Reviews</span>
            </div>
            
            <div className="flex flex-col items-center gap-2">
              <span className="text-7xl font-black text-primary tracking-tighter">4.8<span className="text-3xl text-gray-300 font-bold">/5</span></span>
              <div className="flex gap-1 text-accent">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} size={28} className="fill-accent" />
                ))}
              </div>
              <p className="text-gray-500 font-medium mt-4">Average rating based on 120+ customer reviews.</p>
              <a href="#" className="mt-8 text-accent font-bold hover:underline flex items-center gap-2">
                Write a review <ArrowRight size={16} />
              </a>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
});

GoogleReviews.displayName = 'GoogleReviews';

export default GoogleReviews;
