...
const IndustryPortfolio = memo(() => {
  const [ref, isIntersecting] = useIntersection({ once: true, threshold: 0.1 });

  return (
    <section id="portfolio" ref={ref} className="py-20 md:py-32 bg-white overflow-hidden relative">
      <div className="absolute top-0 right-0 w-64 h-64 bg-accent/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" />
      
      <div className="max-w-7xl mx-auto px-4 md:px-10 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 md:mb-16 gap-6">
          <div className={cn("text-left reveal", isIntersecting && "reveal-active")}>
            <span className="text-accent font-black uppercase tracking-[0.3em] text-[10px] md:text-xs mb-4 block">
        ...
        <div className="flex md:grid md:grid-cols-5 overflow-x-auto md:overflow-x-visible gap-5 md:gap-6 pb-12 md:pb-0 no-scrollbar snap-x snap-mandatory px-4 -mx-4 md:px-0 md:mx-0">
          {industries.map((item, index) => (
            <div
              key={item.name}
              className={cn(
                `p-10 rounded-[3rem] bg-gradient-to-br ${item.color} border border-white/50 backdrop-blur-sm min-w-[300px] md:min-w-0 flex flex-col items-center gap-8 transition-all hover:shadow-premium snap-center group relative overflow-hidden reveal`,
...
IndustryPortfolio.displayName = 'IndustryPortfolio';
